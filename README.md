...
The Homebrew tap is automatically managed using a **dedicated branch** in this repository. No separate repository is required.

#### How It Works

1. **Source files** live in the `homebrew-tap/` directory on the `main` branch
2. **GoReleaser** generates the cask file during releases: `homebrew-tap/Casks/deepscanbot.rb`
3. **GitHub Actions** automatically syncs the cask to the `homebrew-tap` branch
4. **Homebrew** taps the `homebrew-tap` branch for installations
...
- Triggers on every GitHub release
- Extracts the generated cask from `homebrew-tap/Casks/deepscanbot.rb`
- Commits and pushes to the `homebrew-tap` branch
- Makes the cask available via: `brew tap mindfiredigital/DeepScanBot homebrew-tap`
...
1. Verify the release completed successfully
2. Check the `homebrew-tap` branch was updated: `git ls-remote origin homebrew-tap`
3. Wait a few minutes for GitHub to propagate the branch
4. Run `brew update` and try again

**Installation fails:**

1. Check the cask file syntax: `brew audit --cask deepscanbot`
2. Verify the download URL is accessible
3. Check SHA256 checksums match the release artifacts
...
- Syncs Homebrew tap to `homebrew-tap` branch

### Homebrew Sync Workflow (homebrew-sync.yml)

Triggered after every GitHub release:

- Extracts generated cask from `homebrew-tap/Casks/deepscanbot.rb`
- Commits cask to dedicated `homebrew-tap` branch
- Pushes branch to make it available via Homebrew
