...
**Status:** Homebrew installation is currently unavailable until the `mindfiredigital/homebrew-tap` repository is released.

**Alternatives:** Use the [npm installation](#npm-all-platforms) or the [one-liner script](#one-liner) below as temporary installation methods.

```bash
# Coming soon when tap is released:
