...
DeepScanBot comes with a set of features designed for production web crawling.

## Core Features

### Concurrent Crawling

Multi-threaded crawling with configurable concurrency limits.
...
# Use the default of 8 concurrent workers
deepscanbot scan https://example.com
```

The crawler uses 8 concurrent workers by default. You can override this with the `concurrency` option, or pass `concurrency=0` to auto-scale to the number of available CPU cores.
...
The crawler also respects `Retry-After` headers from servers.
...
Route traffic through HTTP or HTTPS proxy servers.
...
deepscanbot scan https://example.com --json show-source=true
```
...
| `host-concurrency`    | `2`                  | Max concurrent requests per host (0 = effective concurrency)     |
